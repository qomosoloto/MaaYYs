...
`自动寮突破 V2（实验版）` 与原版 `自动寮突破` 并行提供，原版流程和入口保持不变。V2 在每场战斗结算后识别突破列表是否真实出现，再按当前进度选择下一个目标，不再依赖正常路径中约 18 秒的固定组合等待。

V2 仍保留以下异常保护：

- 识别短暂出现的“该结界已被攻破”提示。
- 只有成功识别“玩家名 + 进攻按钮”并点击进攻后，才观察同一目标是否持续停留，判断目标可能已被其他成员攻破。
- 点击进攻后等待胜利或失败结算，不使用弹窗背景中仍可见的“击败次数”判断已经返回目标列表。
- 关闭结界突破、重新进入阴阳寮突破并重新选择目标。
- 在运行日志中使用 `寮突破 V2` 前缀记录开始攻击、攻击成功和攻击失败。

建议先单独运行 V2 并观察数轮战斗。若出现识别或恢复异常，停止任务并切回 `自动寮突破`；无需修改配置或删除 V2。状态驱动只能减少不必要的固定等待，不能消除游戏自动化本身的账号与风控风险。
...
验证寮突破 V2 时，选择 `自动寮突破 V2（实验版）`，并重点确认：

1. 结算界面被点击后，突破列表一旦出现就开始选择下一目标，而不是稳定等待约 18 秒。
2. 目标弹窗稳定出现后只点击一次“进攻”，日志出现 `寮突破 V2：开始攻击「玩家名」结界` 及对应成功或失败结果。
3. 只有已经点击进攻的同一目标持续至少 6 秒后，任务才关闭突破界面并重新进入阴阳寮突破。
4. 任务异常时可停止 V2，直接选择原版 `自动寮突破` 回退。
